...
void main() {
  test('rust facade opens sessions and serves query data through the runtime', () async {
    final runtime = _FakeRustLedgerRuntime();
    final bridge = RustBeancountBridgeFacade(runtime: runtime);

    final session = await bridge.openWorkspace('/ledger', '/ledger/main.beancount');
...
class _FakeRustLedgerRuntime implements RustLedgerRuntime {
  ...
  @override
  Future<RustWorkspaceSummary> getWorkspaceSummary({required int handle}) async {
    return _summary();
  }
